...
#                     return 'ok'
def czy_legalny_ruch_damka_biale(xstart, ystart, xend, yend):
    kolor = plansza[xstart][ystart]
    if kolor[0] != ostatni and plansza[xend][yend] == ' ':
...
def czy_legalny_ruch_damka_czarne(xstart, ystart, xend, yend):
    kolor = plansza[xstart][ystart]
    if kolor[0] != ostatni and plansza[xend][yend] == ' ':
        if abs(xstart - xend) == abs(ystart - yend):
            dx = (xend - xstart) // abs(xend - xstart)
...
def wykonaj_ruch(xstart, ystart, xend, yend):
    ...
    if czy_legalny_ruch_damka_biale(xstart, ystart, xend, yend) == 'ok':
        ostatni = k[0]
        plansza[xend][yend] = k
        plansza[xstart][ystart] = ' '
        status = 1
    ...
    for i in range(8):
        for j in range(8):
            if plansza[i][j][0] == 'B':
                ...
                liczba_czarnych += 1
                winner = 2
    # ekran_koncowy(liczba_bialych, liczba_czarnych)
    return False

# def ekran_koncowy(biali, czarni):
#     if biali == 0:
#         napis_wygrana.set("CZARNI WYGRYWAJĄ!")
#         label_wygrana.config(font=("Arial", 40))
#         root.update()
#         root.quit()
#
#         return True
#     elif czarni == 0:
#         napis_wygrana.set("BIALI WYGRYWAJĄ!")
#         label_wygrana.config(font=("Arial", 40))
#         root.update()
#         root.quit()
#         return True
...
pygame.display.set_caption("Warcaby - Koniec Gry")
...
def przycisk(screen, text, color, x, y, width, height):
...
def ekran_koncowy(winner):

    if winner == 2:
        winner_text = "Czarne wygrywają!"
    elif winner == 1:
        winner_text = "Białe wygrywają!"
    else:
        return 'ok'

    wygrany = font.render(winner_text, True, BLACK)
    winner_rect = wygrany.get_rect(center=(400, 200))
    canvas.blit(wygrany, winner_rect)

    przycisk(screen, "Wyjdź", WHITE, 150, 400, 200, 100)
    przycisk(screen, "Zagraj", WHITE, 450, 400, 200, 100)
    ...
    while True:
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if 150 <= mouse_x <= 350 and 400 <= mouse_y <= 500:
                    pygame.quit()
                    sys.exit()
                elif 450 <= mouse_x <= 650 and 400 <= mouse_y <= 500:
                    return
